...
app.put("/api/people/:personID", (req, res) => {
  const id = Number(req.params.personID)//convert that personID route parameter from string to a number
  const { name } = req.body
  if(!id){
...
//Delete person from the list of people using the DELETE method by parsing person
...
app.all("*", (req, res) => {
  res.send("<h1>This route is not available at the moment</h1>")
})
